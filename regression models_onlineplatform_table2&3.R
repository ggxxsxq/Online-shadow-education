...
## city tiers
model1<-lmRob(improvement~gender+course_before+city_tier+scale(gold_coin),data=dta)
summary(model1)
...
## school status 
model3<-lmRob(improvement~gender+course_before+school_status+scale(gold_coin),data=dta)
summary(model3)

## family SES
model4<-lmRob(improvement~gender+course_before+family_ses+scale(gold_coin),data=dta)
summary(model4)
...
# city tiers
model6<-lmRob(scale(post_score)~gender+city_tier+scale(gold_coin)+scale(pre_score)+ course_before
               ,data=dta)
summary(model6)
# city tiers * golden coins
model7<-lmRob(scale(post_score)~gender+city_tier*scale(gold_coin)+scale(pre_score)+ course_before
               ,data=dta)
summary(model7)
...
# school status
model10<-lmRob(scale(post_score)~gender+school_status+scale(gold_coin)+scale(pre_score)+ course_before,data=dta)
summary(model10)
# school status* golden coins
model11<-lmRob(scale(post_score)~gender+school_status*scale(gold_coin)+scale(pre_score)
               + course_before+scale(gold_coin),data=dta)
summary(model11)


## family ses
model12<-lmRob(scale(post_score)~gender+family_ses+scale(gold_coin)+scale(pre_score)+ course_before,data=dta)
summary(model12)
## family ses * golden coins
model13<-lmRob(scale(post_score)~gender+family_ses*scale(gold_coin)+scale(pre_score)
                + course_before+scale(gold_coin),data=dta)
summary(model13)
